...
#define FOSC   22118400L                                     //晶振频率，SysClk对晶振频率不分频
...
void uart_init(void)    //9600bps@22.1184MHz
{
	...
	T2H = HIGH(TM);	    //设定定时初值
	IE2 |= 0x01;        //使能串口2中断
	IP2 &= 0xFE;        //设置中断优先级为优先级0
	P_SW2 |= 0x01;      //设置串口2在P46,P47

	//uart3硬件初始化
	S3CON = 0x10;		//8位数据,可变波特率,接收使能
	S3CON &= 0xBF;		//串口3选择定时器2为波特率发生器
	IE2 |= 0x08;        //使能串口3中断
	P_SW2 |= 0x02;      //设置串口3在P50,P51
	
	//uart4硬件初始化
	S4CON = 0x10;		//8位数据,可变波特率,接收使能
...
//与控制杆通信
void uart2_isr(void) interrupt UART2_VECTOR using 1
{
	Byte c,i;
...
//上位机
void uart3_isr(void) interrupt UART3_VECTOR using 2
{
...
//下位机
void uart4_isr(void) interrupt UART4_VECTOR using 3
{
	volatile Byte nop_num;
	Byte c,i;
